...
class _AddMemberFormState extends State<AddMemberForm> {
  ...
  Future<void> _loadRelations() async {
    try {
      ...
      // Only show snackbar if still mounted and context is valid
  ...
  Future<String?> _sendOtpAndGetVerificationId() async {
    ...
    try {
      ...
      // Only show snackbar if still mounted and has context
      if (mounted && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to send OTP: ${e.toString()}')),
        );
  ...
  //           onResend: _resendOtpInternal,
  //           onDismiss: _clearVerificationState,
  //           onVerificationIdUpdated: (newVerificationId) {
  //             // Update the verification ID when it changes
  //             setState(() {
  ...
  //     });
  //
  //     // Close the current dialog and open a new one with the new verification ID
  //     Navigator.of(context).pop();
  //
  //     // Show new OTP dialog with updated verification ID
  //     await showDialog(
  //       context: context,
  //       barrierDismissible: false,
  //       builder: (context) {
  ...
  // }
  Future<void> _submitForm() async {
    // Check if widget is mounted before any state changes
    if (!mounted) return;
...
class OtpDialogContent extends StatefulWidget {
  ...
  final Future<void> Function() onResend;
  final VoidCallback onDismiss;
  final Function(String) onVerificationIdUpdated;
  const OtpDialogContent({
    super.key,
    required this.phoneNumber,
    required this.verificationId,
    ...
    required this.onResend,
    required this.onDismiss,
    required this.onVerificationIdUpdated,
  });
...
class _OtpDialogContentState extends State<OtpDialogContent> with SingleTickerProviderStateMixin {
  ...
  //
  //   try {
  //     // Call the resend function and get the new verification ID
  //     await widget.onResend();
  //
  //     if (mounted) {
  //       ScaffoldMessenger.of(context).showSnackBar(
  ...
  //  the verification method to use current verification ID
  Future<void> _verifyOtp() async {
    ...
    try {
      ...
      // Use the current verification ID (which might be updated from resend)
      final response = await userModel.validateOtp(_currentVerificationId, otp);
      ...
      if (response['message'] == 'SUCCESS' && response['data']['verificationStatus'] == 'VERIFICATION_COMPLETED') {
        ...
        String errorMsg = 'Invalid OTP. Please check and try again.';
        if (response['message']?.contains('expired') == true || response['message'] == 'VERIFICATION_EXPIRED') {
          errorMsg = 'OTP has expired. Please request a new one.';
        } else if (response['message']?.contains('attempt') == true) {
          errorMsg = 'Too many failed attempts. Please try again later.';
        }
      ...
      if (e.toString().contains('timeout')) {
        ...
        errorMsg = 'Network error. Please check your internet connection.';
      } else if (e.toString().contains('Invalid OTP') || e.toString().contains('VERIFICATION_EXPIRED')) {
        errorMsg = 'The OTP has expired or is incorrect. Please request a new OTP.';
      }
      setState(() => errorMessage = errorMsg);
    } finally {
      if (mounted) {
  ...
  // Method to update verification ID (call this when resend gives new ID)
  void _updateVerificationId(String newVerificationId) {
    if (mounted) {
      setState(() {
