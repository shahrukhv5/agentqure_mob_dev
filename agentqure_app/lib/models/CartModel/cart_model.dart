...
class CartModel extends ChangeNotifier {
  ...
  double get selectedTotalPrice {
    ...
    final total = _requiresHomeCollection
  ...
  Future<void> loadOrderHistory() async {
    try {
      ...
      if (orderHistoryJson != null) {
        ...
        orderHistory = decoded.map<Map<String, dynamic>>((item) {
          return {
            ...item,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _timeSlots = data.asMap().entries.map<Map<String, dynamic>>((entry) {
          ...
          return {...slot, 'timing': slotName, 'available': isAvailable};
        }).toList();
      } else {
        _timeSlots = [];
      }
  ...
  Future<Map<String, dynamic>> placeOrder(String paymentMode) async {
    ...
    try {
      ...
      final walletDiscountPercentage = isWalletEnabled
          ? this.walletDiscountPercentage
          : 0.0;
      ...
      if (paymentMode == 'Pay Now') {
        ...
        remainingAmount = 0.0;
        advancePayment = (actualWalletDiscount + totalAmountPaid)
            .toStringAsFixed(0); // Use actual discount
        ...
        advancePayment = hasWalletDiscount
      ...
      final requestBody = {
        ...
        'raw_booking': {
          ...
          'labPartner': null,
          'discountBy': 'Lab',
          'discountGiven': hasWalletDiscount
              ? actualWalletDiscount.toStringAsFixed(0)
              : '0',
  ...
  Future<void> updateBookingRequestStatus({
    ...
    try {
      ...
      final response = await apiService.post(
        baseUrl: ApiConstants.appUserBaseUrl,
        endpoint: ApiConstants.bookingRequestDecisions,
        data: {
          'request_id': requestId,
