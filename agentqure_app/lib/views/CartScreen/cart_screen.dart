...
class CartScreen extends StatelessWidget {
  ...
  void _showWalletConsumptionBottomSheet(BuildContext context, CartModel cart) {
    ...
    final discountPercentage = isWalletEnabled ? cart.walletDiscountPercentage : 0.0;
    final organizationName = _getOrganizationName(cart);
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInfoPoint(
                      icon: Icons.percent,
                      text: "$organizationName is providing you ${discountPercentage.toStringAsFixed(0)}% discount on your order through wallet points",
                    ),
                    SizedBox(height: 8.h),

                    _buildInfoPoint(
                      icon: Icons.account_balance_wallet,
                      text: "Your available wallet balance will be automatically applied as discount",
                    ),
                    SizedBox(height: 8.h),

                    _buildInfoPoint(
                      icon: Icons.attach_money,
                      text: "If you don't have enough points for the full discount, only your available balance will be utilized",
                    ),
                    SizedBox(height: 8.h),

                    _buildInfoPoint(
                      icon: Icons.payment,
                      text: "The remaining amount after wallet discount can be paid through other payment methods",
                    ),
                    SizedBox(height: 8.h),

                    _buildInfoPoint(
                      icon: Icons.savings,
                      text: "Wallet points never expire and can be used across all services",
                    ),
                    SizedBox(height: 20.h),

                    // Example calculation
                    if (walletBalance > 0 && discountPercentage > 0)
                      Container(
                        padding: EdgeInsets.all(16.w),
                        decoration: BoxDecoration(
                          color: Colors.grey[50],
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 8.h),
                            Text(
                              "On a ₹1000 order with ${discountPercentage.toStringAsFixed(0)}% discount:",
                              style: GoogleFonts.poppins(
                                fontSize: 12.sp,
                                color: Colors.grey[700],
                              ),
                            ...
                            SizedBox(height: 4.h),
                            Text(
                              "• Maximum discount: ₹${(1000 * discountPercentage / 100).toStringAsFixed(0)}",
                              style: GoogleFonts.poppins(
                                fontSize: 12.sp,
                                color: Colors.grey[600],
                              ),
                            ...
                            SizedBox(height: 2.h),
                            Text(
                              "• Actual discount: ₹${(1000 * discountPercentage / 100 <= walletBalance ? (1000 * discountPercentage / 100) : walletBalance).toStringAsFixed(0)} (capped at your balance)",
                              style: GoogleFonts.poppins(
                                fontSize: 12.sp,
                                color: Colors.grey[600],
                              ),
  ...
  Widget _buildWalletInfoRow(String label, String value, {bool isBold = false, Color valueColor = Colors.black87}) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(
  ...
  void _showPaymentOptionsDialog(
    ...
    // FIXED: Calculate wallet discount with proper capping
    final walletDiscount = isWalletEnabled && walletBalance > 0
        ? cart.selectedSubtotal * (cart.walletDiscountPercentage / 100)
        : 0.0;

    // FIXED: Cap the discount at available wallet balance
    final actualWalletDiscount = walletDiscount <= walletBalance
        ? walletDiscount
        : walletBalance;

    // FIXED: Recalculate payable amount with actual discount
    final actualPayableAmount = (cart.selectedSubtotal - actualWalletDiscount) +
              ...
              return WillPopScope(
                ...
                child: Scaffold(
                  ...
                  appBar: AppBar(
                    ...
                    backgroundColor: Colors.grey[200],
                    leading: IconButton(
                      icon: Icon(Icons.arrow_back, color: isLoading ? Colors.grey : Color(0xFF3661E2)),
                    ...
                    children: [
                      SingleChildScrollView(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            // Payment Summary Card (updated with actual values)
                            Container(
                              padding: EdgeInsets.all(16.w),
                              decoration: BoxDecoration(
                                color: Colors.white,
                            ...
                            _buildPaymentOptionCard(
                              ...
                              onTap: isLoading || !hasSufficientBalance
                                  ...
                                  : () {
                                Navigator.pop(context);
                                _initiateRazorpayPayment(cart, actualPayableAmount);
                              },
                            ),
  ...
  void _initiateRazorpayPayment(CartModel cart, double payableAmount) {
    ...
    void handlePaymentSuccess(PaymentSuccessResponse response) async {
      ...
      showDialog(
        ...
        builder: (context) =>
            Center(child: CircularProgressIndicator(color: Color(0xFF3661E2))),
      );
